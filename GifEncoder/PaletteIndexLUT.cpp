	...
	winrt::com_ptr<ID3D11DeviceContext> const& d3dContext)
{
	...
	paletteInfoDesc.ByteWidth = static_cast<uint32_t>(paletteInfoBufferPaddedSize);
...
void PaletteIndexLUT::Generate(std::vector<winrt::Color> const& palette)
{
	WINRT_VERIFY(palette.size() <= 255);

	{
		D3D11_MAPPED_SUBRESOURCE mapped = {};
		winrt::check_hresult(m_d3dContext->Map(m_paletteStagingTexture.get(), 0, D3D11_MAP_WRITE, 0, &mapped));
		memset(mapped.pData, 0, 255);
		auto dest = reinterpret_cast<uint8_t*>(mapped.pData);
		for (auto&& paletteColor : palette)
		{
			dest[0] = paletteColor.R;
			dest[1] = paletteColor.G;
			dest[2] = paletteColor.B;
			dest[3] = paletteColor.A;
